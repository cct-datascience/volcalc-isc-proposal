...
As of today, this issue appears to have been fixed, but now checks on RHub are failing because of [issues with the `RCurl` package](https://cran.r-project.org/web/checks/check_results_RCurl.html) and GitHub actions are failing because of issues [building `ChemmineOB` on macOS](https://github.com/girke-lab/ChemmineOB/issues/35).
